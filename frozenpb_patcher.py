...
def pbtxt_processing(content):
    ...
    if meanCounter > 1:  # May not work
        endMeanNodeName = ""
        print(
            f'Find semi-supported op: Mean presented {meanCounter} times in pb, patching...')
        while 1:
    ...
    if content.find(KEEP_DIM_PATCH.replace('{KEEP_DIM}', 'false')) != -1:
        ...
        while 1:
            keepDimLoc = content.find(
                KEEP_DIM_PATCH.replace('{KEEP_DIM}', 'false'))
            if keepDimLoc == -1:
                break
            ...
            content = content.replace(
                f'input: "{nodeName}"', 'input: "reshape/Reshape"')
            ...
            content = content.replace(KEEP_DIM_PATCH.replace(
                '{KEEP_DIM}', 'false'), KEEP_DIM_PATCH.replace('{KEEP_DIM}', 'true'))
